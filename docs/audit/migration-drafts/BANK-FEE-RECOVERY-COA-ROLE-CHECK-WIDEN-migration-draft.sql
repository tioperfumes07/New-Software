...
-- "_how_to_claim". Mirrors 202613740001's exact pattern (which widened this same constraint for
...
-- itself posted to — 6300 "Bank Service Charges & Wire Fees" — never a new revenue line. Live-
-- verified 2026-09-06 (bypass_rls=lucia): account 6300 exists (id de553cc4-160c-4dec-8256-
-- dfb28e9d4989, account_type Expense).
--
...
-- migration (BANK-FEE-RECOVERY-USMCA-SEED-migration-draft.sql) both land.
--
-- The list below is reproduced from 202613740001 (the most recent widen migration) with the one new
-- value appended. Every existing value is preserved and exactly one is added (Rule 07
-- never-delete-only-add) — RE-VERIFY the live constraint before authoring in case another migration
-- landed in between and added/removed something this snapshot does not reflect.
--
  ...
  IF to_regclass('accounting.chart_of_accounts_roles') IS NOT NULL THEN
    ...
    ALTER TABLE accounting.chart_of_accounts_roles
      ...
      CHECK (role IN (
        ...
        -- NEW — SETL-DED-UI / SETL-DED-GL wire/ACH fee recovery
        'bank_fee_recovery'
      ));
  END IF;
END $$;
